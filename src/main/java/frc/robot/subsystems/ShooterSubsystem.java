...
public class ShooterSubsystem extends SubsystemBase {
    private final SparkFlex m_leftShooter = new SparkFlex(ShooterConstants.kLeftShooterCanId, MotorType.kBrushless);
    private final SparkFlex m_rightShooter = new SparkFlex(ShooterConstants.kRightShooterCanId, MotorType.kBrushless);
    ...
    public ShooterSubsystem() {
        ...
        m_leftShooter.configure(Configs.Shooter.leftShooterConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
        m_rightShooter.configure(Configs.Shooter.rightShooterConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
    ...
    /**
     * A factory function that creates a command to rev the shooter at a specified speed.
     * Will stop the motor when the command ends.
    ...
    /**
     * A factory function that creates a command to rev the shooter at the variable speed.
     * Will continuously update to match variable speed changes and stop when the command ends.
     * @return A Command that revs the shooter at the current variable speed.
     */
    public Command getRevShooterVariableCommand() {
        return Commands.run(
            () -> m_shooterSpeed = m_variableSpeed,
            this)
        .finallyDo(() -> m_shooterSpeed = 0);
    }

    /**
     * A factory function that creates a command to increment the variable speed.
     * Does not require the subsystem, so it won't interrupt the shooter.
    ...
    /**
     * A factory function that creates a command to decrement the variable speed.
     * Does not require the subsystem, so it won't interrupt the shooter.
    ...
    /**
     * A factory function that creates a command to reset the variable speed to 50%.
     * Does not require the subsystem, so it won't interrupt the shooter.
    ...
    /**
     * A factory function that creates a command to run the shooter in reverse to unclog.
     * Will stop the motor when the command ends.
